...
export function ThreatAnalytics({ threats }: ThreatAnalyticsProps) {
  ...
  const getSourceIcon = (source: string) => {
    ...
    if (normalized.includes("cisa")) return Shield;
    if (normalized.includes("hacker news")) return Newspaper;
    return null;
  };
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
              ...
              return (
                <div key={level} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className={`font-medium ${getRiskColor(level)}`}>{level}</span>
                  ...
                  <div className="h-2 rounded-full bg-secondary/60 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${severityBarColor[level]} transition-all duration-700 ease-out`}
                      style={{
        ...
        <CardContent>
          <div className="space-y-3">
            {Object.entries(typeDistribution)
              ...
              .slice(0, 5)
              .map(([type, count], i) => {
                const typedType = type as Parameters<typeof getThreatTypeIcon>[0];
                  ...
                  <div key={type} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className="text-muted-foreground shrink-0 ml-2 tabular-nums">{count}</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-secondary/60 overflow-hidden">
                        <div
                          className="h-full rounded-full bg-teal-500/70 transition-all duration-700 ease-out"
                          style={{
                            width: animated ? `${percentage}%` : "0%",
      ...
      <Card className="border-border/50 bg-card/80 backdrop-blur-sm md:col-span-2">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold">Intelligence Sources</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2.5">
            {Object.entries(sourceDistribution).map(([source, count]) => {
              ...
              return (
                <div
                  key={source}
                  className="flex items-center gap-2 rounded-xl border border-border/50 bg-secondary/30 px-3 py-2 transition-colors duration-200 hover:bg-secondary/60"
                >
                  {SourceIcon ? (
                    <SourceIcon className="h-3.5 w-3.5 text-teal-400 shrink-0" />
                  ) : (
